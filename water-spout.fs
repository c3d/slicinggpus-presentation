...
uniform float iWaterVelocity;

#define time iGlobalTime
#define mouse (iMouse.xy/iResolution.xy*vec2(0.9, 0.5)-vec2(0.25,0.5))
#define resolution iResolution
#define viewpoint iViewpoint
#define water_velocity iWaterVelocity

// Spout - @P_Malin
// Original here: http://www.shadertoy.com/view/lsXGzH
...
const float kWaterNoiseScale = 0.035;

float kWaterVelocity = water_velocity;

const float kWaterAccel = -1.0;

const float kWaterAnimSpeed = 81.712831;
const float kTrenchWaterAnimSpeed = 47.0;



float kRipplePos = sqrt(abs(2.0 * kPipeHeight / kWaterAccel)) * kWaterVelocity;
float kRippleScale = 8.9;
...
{
    float l = length(p);
    vec2 s = sin(0.5 * vec2(l)) + 0.1 * sin(vec2(1.23192318,1.123123)*p);
...
{          
	...
	float fNoiseScale = mix(t * t, kRippleScale / (1.0 + vNoiseDomain.x), fInTrench) * kWaterNoiseScale;
...
C_Material GetObjectMaterial( const in C_HitInfo hitInfo )
{
    ...
    mat.fRefractiveIndex = 1.0;
    if(hitInfo.vObjectId.x == kMaterialIdWall)
    {
        // floor
        mat.fR0 = 0.02;
        vec2 xy = hitInfo.vObjectId.yz;
	vec2 vTile = smoothstep(vec2(0.1), vec2(0.11), fract(xy));
	float fTile = vTile.x * vTile.y;
        float xx = ceil(xy.x);
        float yy = ceil(xy.y);
        vec3 colorTile = fract(xx * vec3(1231.1141,414243.3312,3231.2231) + yy*vec3(1312.3,333.3,3213.2));
        vec3 colorLine = vec3(0.2);
        mat.cAlbedo = mix(colorLine, colorTile, fTile);
        mat.fSmoothness = 1.0;
...
{        
    ...
    result.vObjectId = vec3(0.0);
    result.vPos = vec3(0.0);
...
{                                                          
    ...
    else
    {
        ...
        {
            surface.cTransmission = vec3(0.0);
        }
...
void main( void )
{
    ...
    float fHeading = mix(-0.5, kPI + 0.5, mouse.x + viewpoint);
