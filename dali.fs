...
// This is slow with reflections on some GPUs. Enable reflections below for the full effect.
...
float kPI = 3.14159265858;
float kPIDiv2 = kPI / 2.0;
...
void CalcPixelRandom()
{
    ...
    vec4 s1 = sin(iGlobalTime * 3.3422 + gl_FragCoord.xxxx * vec4(324.324234, 563.324234, 657.324234, 764.324234)) * 543.3423;
    vec4 s2 = sin(iGlobalTime * 1.3422 + gl_FragCoord.yyyy * vec4(567.324234, 435.324234, 432.324234, 657.324234)) * 654.5423;
...
void AddHand(float time, float circle, float Id, float Height, float Thickness, float Base, float Length, const in vec3 vPos, inout vec4 vResult)
{
	const float fThickness = 0.1;
	const float fInsetDepth = 0.1;

	float fHandSeconds = time;
	float fHandAngle = kPI - fHandSeconds / circle * kTwoPI;

	vec3 vHandDomain = RotateZ(vPos, fHandAngle);
	float fHandHeight = Height;
	vHandDomain.z -= fThickness - fInsetDepth + fHandHeight;
	float fHandDist = length(vHandDomain.xz) - Thickness;
	fHandDist = max(fHandDist, (abs(vHandDomain.y + Base) - Length));

	vec4 vHandDist = vec4(fHandDist, Id, 0.0, 0.0);
...
vec4 GetDistanceClock( const in vec3 vPos )
{
	...
	float fHandSeconds = floor(iGlobalTime) + (pow(fract(iGlobalTime), 10.0));

        AddHand(fHandSeconds, 60.0,         4.0, 0.07, 0.005, 0.4, 0.3, vPos, vResult);
        AddHand(iGlobalTime/60.0, 60.0,    2.0, 0.06, 0.02, 0.25, 0.3, vPos, vResult);
        AddHand(iGlobalTime/3600.0, 12.0,  2.0, 0.03, 0.03, 0.15, 0.2, vPos, vResult);
...
vec3 WarpDomain( const in vec3 vPos )
{
    ...
    float fUnbend = clamp(atan(vResult.y, vResult.z), 0.0, kPIDiv2 * 0.9);
...
C_Material GetObjectMaterial( const in vec3 vObjId, const in vec3 vPos, const in vec3 vNormal )
{
    ...
    if(vObjId.x < 1.5)
    {
        // wall
        mat.fR0 = 0.2;
        vec3 cTextureSample = texture2D(iChannel0, vObjId.yz * 0.25).rgb;
    ...
    if(vObjId.x < 5.5)
    {
        // floor
        mat.fR0 = 0.01;
		vec3 cTextureSample = texture2D(iChannel1, vObjId.yz*0.3).rgb;
        mat.cAlbedo = cTextureSample * cTextureSample;
...
void main( void )
{
    ...
    GetCameraRayLookat( vCameraPos, vec3(-0.05*iViewpoint, -0.5, 0.2), ray);
	...
	// vignette
	vec2 vUV = ((gl_FragCoord.xy / iResolution.xy) - 0.5) * 2.0;
