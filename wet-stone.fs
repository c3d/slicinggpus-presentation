...
float rock(vec3 p) {    
    ...
    for(int i = 0; i < 9; i++) {
        float ii = float(i);
        float r = 2.5 + hash11(ii + floor(iGlobalTime * (0.0133 + 0.021 * float(i))));
...
vec3 getStoneColor(vec3 p, float c, vec3 l, vec3 n, vec3 e) {
    ...
    vec3 base = vec3(0.42,0.3,0.2) * (0.7 + 0.2 * sin(floor(iGlobalTime*0.11)));
    vec3 delta = vec3(0.2, 0.1, 0.08) * sin(vec3(3.11, 2.14, 0.28) * sin(floor(iGlobalTime * 0.0523)));
    vec3 sand = vec3(0.51,0.41,0.32);
    vec3 color = mix(base+delta,sand+delta,c);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // ray
    vec3 ang = vec3(0.0,0.2,time - 0.1 * iViewpoint);
