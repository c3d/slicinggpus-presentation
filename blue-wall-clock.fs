...
const vec3 light2 = vec3(-5.5, 22.0, 12.5);
...
void Intersect(Ray r, inout Intersection i)
{
    ...
    Sphere s;
    for (float j = 10.0; j < 11.2; j += 0.1)
    {
        s.c = vec3(0.0, 3.0, 3.0) + vec3(3.0, 2.8, 2.0) * sin(vec3(1.117, 0.5111, 2.118) * (time - 2.1*j));
        s.r = 0.3 + 0.25 * sin(0.21 * (time-2.1*j));
        s.col =  vec4(0.8) + 0.4 * sin(vec4(0.31, 0.13, 0.22, 0.0) * (time - 12.1 * j));
        sphere_intersect(s, r, i);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float angle = mouse.x * 0.1 - 10.1 + 60.1 * sin(0.1 * time);
    float radius = 20.5 - 0.05 * mouse.y - 12.24 * (1.0-pow(0.5+0.5*sin(0.031 * time), 17.0));

    float ss = sin(angle * todeg);
    float cc = cos(angle * todeg);
    vec3 org = vec3(ss*radius - 0.2 * iViewpoint,2.0,cc*radius);
    vec3 dir = normalize(vec3(position.x*cc-ss,position.y, -position.x*ss-cc));

    vec3 center = vec3(-1.0, pow(0.5+0.5*sin(3.1415926*time),15.0)+1.5, 2.75);
    sphere[0].c   = center;
    sphere[0].r   = 0.5;
